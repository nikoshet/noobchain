...
class Transaction:
    ...
    def __init__(self, sender_address, receiver_address, amount, transaction_inputs, wallet,id, genesis=False):
        ...
        if not genesis:
            ...
            self.transaction_outputs.append({str(self.node_id)+str(self.transaction_id):(self.receiver_address,self.amount)})
            Transaction._id += 1
            self.transaction_id = Transaction._id 
            self.transaction_outputs.append({str(self.node_id)+str(self.transaction_id):(self.receiver_address,self.change)})
            print(self.transaction_outputs)
        else:
